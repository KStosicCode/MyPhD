...
BiocManager::install(c("GSVA", "sva"))
...
#Define custom colors for the groups (switched)
group_colors <- c("LessThan12Months" = "#D8BFD8", "MoreThan36Months" = "#B4E197")

#Define the common theme with white background and black outline
custom_theme <- theme(
...
custom_theme <- theme(
  ...
  plot.background = element_rect(fill = "white", color = "black"),  # Overall plot white background
  panel.grid = element_blank(),  # Remove gridlines for a clean look
...
#T cells
t <- ggplot(metadata, aes(x = OS, y = T.cell, fill = OS)) + 
  geom_boxplot() +
  ggtitle("T cells") +
  theme(plot.title = element_text(hjust = 0.5)) +
  geom_signif(comparisons = list(c("LessThan12Months", "MoreThan36Months")), 
...
t <- ggplot(metadata, aes(x = OS, y = T.cell, fill = OS)) + 
  ...
  geom_jitter(width = 0.2, size = 1.5, alpha = 0.6, color = "lightgrey") +  # Add jittered points in light grey
  ggtitle("T cells") +
  geom_signif(comparisons = list(c("LessThan12Months", "MoreThan36Months")), 
              map_signif_level = TRUE) +
  scale_fill_manual(values = group_colors) +
...
t <- ggplot(metadata, aes(x = OS, y = T.cell, fill = OS)) + 
  geom_boxplot(outlier.shape = NA) +  # Suppress default outliers
  geom_jitter(width = 0.2, size = 1.5, alpha = 0.8, color = "#A9A9A9") +  # Medium grey dots
  ggtitle("T cells") +
  geom_signif(comparisons = list(c("LessThan12Months", "MoreThan36Months")), 
              map_signif_level = TRUE) +
  scale_fill_manual(values = group_colors) +
...
#B cells
b <- ggplot(metadata, aes(x = OS, y = B.Cell, fill = OS)) + 
  geom_boxplot() +
  ggtitle("B Cells") +
  theme(plot.title = element_text(hjust = 0.5)) +
  geom_signif(comparisons = list(c("LessThan12Months", "MoreThan36Months")), 
...
#Myeloid dendritic cell
myeloid_dendritic_cell <- ggplot(metadata, aes(x = OS, y = Myeloid.dendritic.cell, fill = OS)) + 
  geom_boxplot() +
  ggtitle("Myeloid Dendritic Cell") +
  theme(plot.title = element_text(hjust = 0.5)) +
  geom_signif(comparisons = list(c("LessThan12Months", "MoreThan36Months")), 
...
#Plot T cells, B cells and Myeloid Dencritic Cells together
install.packages("patchwork")
library(patchwork)
#Combine all three plots at once and collect the legend into one shared space
combined_plot <- (t + b + myeloid_dendritic_cell) +
  plot_layout(ncol = 3, guides = "collect") +  
  plot_annotation(title = "MCP Counter")  
